"""Read-only Stage 10.3 composition over frozen Stage 3 and Stage 9 data."""
...
from spy_research.strategy.comparisons.models import VwapAlignmentComparisonResult
...
class VwapAlignmentComparisonService:
    """Attach exact confirmation-bar price/VWAP labels to frozen outcomes."""
    ...
    def calculate(self, *, start: date, end: date) -> VwapAlignmentComparisonResult:
        ...
        annotations = annotate_confirmed_vwap_alignment(
        ...
        cross_annotations = annotate_prior_cross_context(
            setup_result, cross_result.events
        )
        return calculate_vwap_alignment_comparison(
            setup_result,
            outcome_result,
            base_statistics,
            annotations,
