"""Deterministic daily-reset EMA20/VWAP completed-candle cross events."""
...
class Ema20VwapCrossInputError(ValueError):
    """Accepted EMA20/VWAP rows cannot form a trustworthy event universe."""


def ema20_vwap_event_identity(
    symbol: str,
    timestamp,
...
def detect_ema20_vwap_crosses(
    ema_rows: Sequence[FiveMinuteIndicatorRow],
    vwap_rows: Sequence[FiveMinuteVwapRow],
) -> tuple[tuple[Ema20VwapCrossEvent, ...], tuple[Ema20VwapCrossSessionSummary, ...]]:
    ...
    def index(rows, label):
        ...
        for row in rows:
            ...
            if key in indexed:
                raise Ema20VwapCrossInputError(f"Duplicate {label} timestamp")
            if previous is not None and row.timestamp <= previous:
                raise Ema20VwapCrossInputError(f"{label} rows must be chronological")
            if row.timeframe != "5Min" or row.session_mode != "RTH_ONLY":
                raise Ema20VwapCrossInputError(f"{label} rows require RTH 5Min provenance")
            indexed[key] = row
    ...
    if set(ema_by_key) != set(vwap_by_key):
        raise Ema20VwapCrossInputError("EMA20 and VWAP timestamp universes must match")
    ...
    for key, ema in ema_by_key.items():
        ...
        if ema.symbol != vwap.symbol:
            raise Ema20VwapCrossInputError("EMA20/VWAP symbols must match")
    ...
    for session_date, rows in grouped.items():
        ...
        for (prior_ema, prior_vwap), (current_ema, current_vwap) in zip(
            ...
            if current_ema.timestamp - prior_ema.timestamp != timedelta(minutes=5):
                raise Ema20VwapCrossInputError(
                    "EMA20/VWAP cross rows must be adjacent five-minute buckets"
                )
            values = (
                prior_ema.ema20,
                prior_vwap.vwap,
            ...
            if any(value is None for value in values):
                continue
            assert prior_ema.ema20 is not None
            assert prior_vwap.vwap is not None
            assert current_ema.ema20 is not None
            assert current_vwap.vwap is not None
            direction = None
            if (
                current_ema.ema20 > current_vwap.vwap
                and prior_ema.ema20 <= prior_vwap.vwap
            ):
                direction = Ema20VwapCrossDirection.BULLISH
            elif (
                current_ema.ema20 < current_vwap.vwap
                and prior_ema.ema20 >= prior_vwap.vwap
            ):
                direction = Ema20VwapCrossDirection.BEARISH
            if direction is None:
                continue
            with localcontext(VWAP_CONTEXT):
                signed = current_ema.ema20 - current_vwap.vwap
            event = Ema20VwapCrossEvent(
                event_identity=ema20_vwap_event_identity(
                ...
                ema20=current_ema.ema20,
                vwap=current_vwap.vwap,
                prior_ema20=prior_ema.ema20,
        ...
        summaries.append(
            Ema20VwapCrossSessionSummary(
                session_date=session_date,
                bullish_crosses=sum(
                    item.direction is Ema20VwapCrossDirection.BULLISH
                    for item in session_events
                ),
                bearish_crosses=sum(
                    item.direction is Ema20VwapCrossDirection.BEARISH
                    for item in session_events
                ),
    ...
    if len({item.event_identity for item in events}) != len(events):
        raise Ema20VwapCrossInputError("Duplicate EMA20/VWAP event identity")
