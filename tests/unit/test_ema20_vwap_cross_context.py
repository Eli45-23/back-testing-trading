...
def rows(values, *, start=START, session=SESSION):
    ema_rows = []
    vwap_rows = []
    for index, (ema20, vwap) in enumerate(values):
...
@pytest.mark.parametrize(
    "values,direction",
    (
        ((("100", "100"), ("101", "100")), Ema20VwapCrossDirection.BULLISH),
        ((("100", "100"), ("99", "100")), Ema20VwapCrossDirection.BEARISH),
        ((("99", "100"), ("101", "100")), Ema20VwapCrossDirection.BULLISH),
        ((("101", "100"), ("99", "100")), Ema20VwapCrossDirection.BEARISH),
    ),
)
def test_cross_definition_including_equality(values, direction) -> None:
    ema_rows, vwap_rows = rows(values)
    events, _ = detect_ema20_vwap_crosses(ema_rows, vwap_rows)
    assert len(events) == 1
...
def test_persistent_relationship_does_not_repeat(values) -> None:
    events, _ = detect_ema20_vwap_crosses(*rows(values))
    assert events == ()
...
def test_event_identity_is_deterministic() -> None:
    ...
    first = detect_ema20_vwap_crosses(*source)[0]
    second = detect_ema20_vwap_crosses(*source)[0]
    assert first == second
...
def test_cross_detection_resets_without_overnight_bridge() -> None:
    ...
    events, summaries = detect_ema20_vwap_crosses(
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            SetupDirection.LONG,
            Ema20VwapCrossDirection.BEARISH,
...
def test_directional_context_mapping(direction, cross_direction, expected) -> None:
    values = (
        (("99", "100"), ("101", "100"))
        if cross_direction is Ema20VwapCrossDirection.BULLISH
        else (("101", "100"), ("99", "100"))
    )
    event = detect_ema20_vwap_crosses(*rows(values))[0][0]
    annotation = select_prior_ema20_vwap_cross(setup(direction), (event,))
...
def test_exact_recency(bars) -> None:
    event = detect_ema20_vwap_crosses(
        *rows((("99", "100"), ("101", "100")))
    )[0][0]
    item = setup(confirmation=event.cross_timestamp + timedelta(minutes=bars * 5))
    annotation = select_prior_ema20_vwap_cross(item, (event,))
...
def test_future_cross_and_later_cross_cannot_change_prior_annotation() -> None:
    events = detect_ema20_vwap_crosses(
        *rows((("99", "100"), ("101", "100"), ("99", "100")))
    )[0]
    item = setup(confirmation=events[0].cross_timestamp)
    assert len(events) == 2
    first = select_prior_ema20_vwap_cross(item, events[:1])
    second = select_prior_ema20_vwap_cross(item, events)
    assert first == second
...
def test_newer_opposing_cross_overrides_older_matching_cross() -> None:
    ...
    annotation = select_prior_ema20_vwap_cross(item, events)
    assert annotation.cross_timestamp == events[-1].cross_timestamp
    assert annotation.cross_state is Ema20VwapCrossContextState.OPPOSING_EMA20_VWAP_CROSS


def test_no_prior_cross_is_explicit() -> None:
    annotation = select_prior_ema20_vwap_cross(setup(), ())
    assert annotation.cross_state is Ema20VwapCrossContextState.NO_PRIOR_EMA20_VWAP_CROSS
...
def test_detector_and_context_are_offline_nonpersistent(monkeypatch, tmp_path) -> None:
    def reject_network(*args, **kwargs):
        raise AssertionError("Stage 10.7 must remain offline")

    monkeypatch.setattr(socket, "create_connection", reject_network)
    events = detect_ema20_vwap_crosses(
        *rows((("99", "100"), ("101", "100")))
    )[0]
    select_prior_ema20_vwap_cross(setup(), events)
...
def test_cli_is_offline_and_read_only(monkeypatch, tmp_path, capsys) -> None:
    ...
    def mocked_print(result):
        ...
        print("ema20-vwap-cross-pass")

    monkeypatch.setattr(
        Ema20VwapCrossContextComparisonService, "calculate", mocked_calculate
    )
    monkeypatch.setattr(
        cli_module, "_print_ema20_vwap_cross_context_comparison", mocked_print
    )
    ...
    assert exit_code == 0
    assert captured.out == "ema20-vwap-cross-pass\n"
    assert captured.err == ""
    assert list(tmp_path.iterdir()) == []
